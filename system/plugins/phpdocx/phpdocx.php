...
class phpdocx {
    ...
    private $template;
    private $tmpDir = "cache/phpdocx"; // must be writable
    ...
    private function cleanDuplicatedLayout() {
        ...
        while ( ($wro_p=$this->foundTag($this->content, $wro, $wro_p))!==false ) {
            $wto_p = $this->foundTag($this->content,$wto,$wro_p);
            if ($wto_p===false) return false; // error in the structure of the <w:r> element
            $first = true;
            do {
                $ok = false;
                $wtc_p = $this->foundTag($this->content,$wtc,$wto_p);
                if ($wtc_p===false) return false; // error in the structure of the <w:r> element
                $wrc_p = $this->foundTag($this->content,$wrc,$wro_p);
                if ($wrc_p===false) return false; // error in the structure of the <w:r> element
