...
class m_registration extends CI_Model {
    ...
    function get_detail_data_by_id($params) {
        $sql = "SELECT * FROM fa_data WHERE data_id = ? AND airlines_id = ? AND data_st = 'open'";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
    ...
    // get izin rute
    function get_izin_rute($params) {
        $sql = "SELECT airlines_id, kode_izin, izin_approval, izin_type, izin_flight, izin_active, izin_st, izin_rute_start, izin_rute_end, pairing 
            FROM izin_rute 
            WHERE airlines_id = ? AND izin_flight = ? AND izin_approval = 'approved' AND izin_active != '0' AND (izin_st != 'pencabutan' OR izin_st != 'penundaan') 
            GROUP BY kode_izin, pairing";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
    ...
    // time converter
    function get_airport_utc($params) {
        $sql = "SELECT airport_nm, airport_iata_cd, airport_region, airport_utc_sign, airport_utc, airport_utc_2 
            FROM airport 
            WHERE airport_iata_cd = ? OR airport_region = ?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
