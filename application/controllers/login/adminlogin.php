...
class adminlogin extends ApplicationBase {
    ...
    public function index($status = "") {
                ...
                $this->tsession->set_userdata('session_adminpk', 1);
    ...
    public function login_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if (!empty($result)) {
                ...
                // set session
                $this->tsession->set_userdata('session_adminpk', $result['user_id']);
                // insert login time
    ...
    public function logout_process() {
        // user id
        $user_id = $this->tsession->userdata('session_adminpk');
        // insert logout time
        $this->m_account->update_user_logout($user_id);
        // unset session
        $this->tsession->unset_userdata('session_adminpk');
        // output
        redirect('login/adminlogin');
    }
