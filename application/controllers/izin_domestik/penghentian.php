...
class penghentian extends ApplicationBase {
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_domestik/penghentian/index.html");
        // load javascript
        ...
        // list izin rute
        $rs_id = $this->m_registrasi->get_list_izin_rute_all_by_airlines(array($this->com_user['airlines_id'], 'domestik'));
    ...
    public function edit_process() {
        ...
        // detail
        $detail = $this->m_registrasi->get_detail_izin_rute_by_rute_all(array($this->com_user['airlines_id'], 'domestik', $this->input->post('izin_rute_start')));
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_registrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_domestik/penghentian/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
        ...
        // default redirect
        redirect("izin_domestik/penghentian/index/" . $this->input->post('registrasi_id'));
    }
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_domestik/penghentian/list.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_registrasi->get_registrasi_waiting_by_id($params);
        if (empty($result)) {
    ...
    public function list_rute_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // redirect
            redirect("izin_domestik/penghentian/list_files/" . $this->input->post('registrasi_id'));
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function list_files($registrasi_id = "") {
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "izin_domestik/penghentian/files.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_registrasi->get_registrasi_waiting_by_id($params);
        if (empty($result)) {
    ...
    public function files_process() {
        ...
        // default redirect
        redirect("izin_domestik/penghentian/list_files/" . $registrasi_id);
    }
    ...
    public function files_download($data_id = "", $ref_id = "") {
        ...
        if (empty($result)) {
            redirect("izin_domestik/penghentian/list_files/" . $data_id);
        }
        // filepath
    ...
    public function files_next() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // default redirect
            redirect("izin_domestik/penghentian/review/" . $registrasi_id);
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function review($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_domestik/penghentian/review.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_registrasi->get_registrasi_waiting_by_id($params);
        if (empty($result)) {
    ...
    public function send_process() {
        ...
        // default redirect
        redirect("izin_domestik/penghentian/review/" . $data_id);
    }
    ...
    function get_rute_all_by_rute() {
        ...
        $result = $this->m_registrasi->get_detail_izin_rute_by_rute_all(array($this->com_user['airlines_id'], 'domestik', $izin_rute_start));
        if (!empty($result)) {
            $result['pax_cargo'] = strtoupper($result['pax_cargo']);
            $result['masa_berlaku'] = $this->datetimemanipulation->get_full_date($result['izin_start_date'], 'ins') . ' / ' . $this->datetimemanipulation->get_full_date($result['izin_expired_date'], 'ins');
            $result['masa_berlaku'] = strtoupper($result['masa_berlaku']);
