...
class registration_internasional extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "member/registration_internasional/index.html");
        // list group
        $rs_id = $this->m_izin->get_list_group_by_flight('internasional');
        $this->smarty->assign("rs_group", $rs_id);
        // list opened form
        $rs_id = $this->m_izin->get_list_registration_open(array('internasional', $this->com_user['airlines_id']));
    ...
    public function disclaimer_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->session->userdata('word') == $captcha AND $this->session->userdata('ip_address') == $_SERVER["REMOTE_ADDR"] AND $this->session->userdata('captcha_time') > $expiration) {
                ...
                // redirect
                redirect("member/registration_internasional/create/" . $group_id);
            } else {
        ...
        // redirect
        redirect("member/registration_internasional/disclaimer/" . $group_id);
    }
