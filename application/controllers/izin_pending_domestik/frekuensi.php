...
class frekuensi extends ApplicationBase {
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/index.html");
        // load javascript
    ...
    public function edit_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_izin->update_izin_permohonan($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_domestik/frekuensi/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
        ...
        // default redirect
        redirect("izin_pending_domestik/frekuensi/index/" . $this->input->post('registrasi_id'));
    }
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/list.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        // load style ui
        $this->smarty->load_style("jquery.ui/redmond/jquery-ui-1.8.13.custom.css");
    ...
    public function list_rute_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if (@intval($rs_airport['is_all_used_score']) !== 1){
                redirect("izin_pending_domestik/frekuensi/list_slot/" . $this->input->post('registrasi_id'));
            }else{
                redirect("izin_pending_domestik/frekuensi/list_files/" . $this->input->post('registrasi_id'));
            }
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
    ...
    public function edit_rute_score_next(){
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($pairing == "VV"){
                ...
                // show next step
                $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/edit_score_next.html");
                // load javascript
                $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
                $this->smarty->load_javascript("resource/js/jquery/jquery.ui.timepicker.js");
                $this->smarty->load_javascript("resource/js/select2-3.4.5/select2.min.js");
                ...
                if ($this->m_izin->insert_rute($params)) {
                    ...
                    $this->tnotification->delete_last_field();
                    $this->tnotification->sent_notification("success", "Data berhasil disimpan");
                    if ($rs_airport1['is_used_score'] != 1 || $rs_airport2['is_used_score'] != 1) redirect("izin_pending_domestik/frekuensi/rute_edit/" . $registrasi_id . "/" . $kode_frekuensi . "/form");
                } else {
                    // default error
                    $this->tnotification->sent_notification("error", "Data gagal disimpan");
                }
        ...
        // default redirect
        redirect("izin_pending_domestik/frekuensi/rute_edit/" . $registrasi_id . '/' . $kode_frekuensi);
    }
    ...
    public function edit_rute_process() {
        ...
        // default redirect
        if ($mode == "form")
            redirect("izin_pending_domestik/frekuensi/rute_edit/" . $this->input->post('registrasi_id') . '/' . $kode_frekuensi . '/form');
        else
            redirect("izin_pending_domestik/frekuensi/rute_edit/" . $this->input->post('registrasi_id') . '/' . $kode_frekuensi);
    }
    ...
    public function list_slot($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/list_slot.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_izin->get_pending_registrasi_by_id($params);
        if (empty($result)) {
    ...
    public function slot_add($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/slot_add.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        // load style ui
        $this->smarty->load_style("jquery.ui/redmond/jquery-ui-1.8.13.custom.css");
    ...
    public function add_slot_process() {
        ...
        // default redirect
        redirect("izin_pending_domestik/frekuensi/slot_add/" . $this->input->post('registrasi_id'));
    }
    ...
    public function slot_edit($registrasi_id = "", $slot_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/slot_edit.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        // load style ui
        $this->smarty->load_style("jquery.ui/redmond/jquery-ui-1.8.13.custom.css");
    ...
    public function edit_slot_process() {
        ...
        $result = $this->m_izin->get_detail_slot_by_id(array($registrasi_id, $slot_id));
        if (empty($result)) {
            redirect('izin_pending_domestik/frekuensi/list_slot/' . $registrasi_id);
        }
        // process
        if ($this->tnotification->run() !== FALSE) {
        ...
        // default redirect
        redirect("izin_pending_domestik/frekuensi/slot_edit/" . $registrasi_id . '/' . $slot_id);
    }
    ...
    public function list_files($registrasi_id = "") {
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/files.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_izin->get_pending_registrasi_by_id($params);
        if (empty($result)) {
    ...
    public function files_process() {
        ...
        // default redirect
        redirect("izin_pending_domestik/frekuensi/list_files/" . $registrasi_id);
    }
    ...
    public function files_download($data_id = "", $ref_id = "") {
        ...
        if (empty($result)) {
            redirect("izin_pending_domestik/frekuensi/list_files/" . $data_id);
        }
        // filepath
    ...
    public function files_next() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // default redirect
            redirect("izin_pending_domestik/frekuensi/review/" . $registrasi_id);
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function review($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/frekuensi/review.html");
        /// get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_izin->get_pending_registrasi_by_id($params);
        if (empty($result)) {
    ...
    public function batalkan($registrasi_id = "") {
        ...
        if (empty($result)) {
            redirect('izin_pending_domestik/frekuensi');
        }
        // params
        $params = array(
            "izin_completed"    => "1",
