...
class statistik_fa extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "report/statistik_fa/index.html");
        // load javascript
        ...
        $rs_id = $this->m_report_statistik->get_data_statistik_pelayanan($params);
        $data = array();
        $total = 0;
        $total_awaiting = 0;
        $total_approved = 0;
        ...
        // belum diproses
        // statistik
        $rs_id = $this->m_report_statistik->get_data_statistik_pelayanan_belum_proses($params);
        $data = array();
        $total = 0;
        foreach ($rs_id as $value) {
    ...
    public function data_chart() {
        ...
        // BERJADWAL
        $rs_id = $this->m_report_statistik->get_data_chart_fa(array('berjadwal'));
        $data_berjadwal = array();
        foreach ($rs_id as $data) {
            $data_berjadwal[$data['bulan']] = $data['total'];
        }
        // TIDAK BERJADWAL
        $rs_id = $this->m_report_statistik->get_data_chart_fa(array('tidak berjadwal'));
        $data_tidak_berjadwal = array();
        foreach ($rs_id as $data) {
            $data_tidak_berjadwal[$data['bulan']] = $data['total'];
        }
        // BUKAN NIAGA
        $rs_id = $this->m_report_statistik->get_data_chart_fa(array('bukan niaga'));
    ...
    public function data_chart_pie() {
        ...
        // BUKAN NIAGA
        $rs_id = $this->m_report_statistik->get_data_chart_pie($params);
        $data = array();
        foreach ($rs_id as $value) {
