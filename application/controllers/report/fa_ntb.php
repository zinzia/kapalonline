...
class fa_ntb extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('m_report_fa_ntb');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "report/fa_ntb/index.html");
        // load javascript
        ...
        $search = $this->tsession->userdata('search_report_ntb');
        if (!empty($search)) {
        ...
        $data_type = empty($search['data_type']) ? '%tidak berjadwal%' : '%' . $search['data_type'] . '%';
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("report/fa/index/");
        $config['total_rows'] = $this->m_report_fa_ntb->get_total_report(array($tanggal_from, $tanggal_to, $tanggal_from, $tanggal_to, $published_no, $published_no, $data_type, $data_flight, $payment_st, $airlines_nm, $services_cd, 3, 4, 5, 6));
        ...
        /* end of pagination ---------------------- */
        // get list
        $params = array($tanggal_from, $tanggal_to, $tanggal_from, $tanggal_to, $published_no, $published_no, $data_type, $data_flight, $payment_st, $airlines_nm, $services_cd, 3, 4, 5, 6, ($start - 1), $config['per_page']);
        $rs_id = $this->m_report_fa_ntb->get_list_report($params);
        $this->smarty->assign("rs_id", $rs_id);
        // get airport
        $this->smarty->assign("rs_airport", $this->m_report_fa_ntb->get_all_airport());
        // get services code
        $this->smarty->assign("rs_services_code", $this->m_report_fa_ntb->get_all_services_code());
        // get list airlines
        $this->smarty->assign("rs_airlines", $this->m_report_fa_ntb->get_list_airlines());
        // get list services
        $this->smarty->assign("rs_services", $this->m_report_fa_ntb->get_list_services());
        // output
        parent::display();
    }
    ...
    public function proses_cari() {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('search_report_ntb');
        } else {
            $params = array(
            ...
            $this->tsession->set_userdata("search_report_ntb", $params);
        }
        // redirect
        redirect("report/fa_ntb");
    }
    ...
    public function form($data_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "report/fa_ntb/form.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        $this->smarty->load_javascript("resource/js/select2-3.4.5/select2.min.js");
        // load style ui
        ...
        $result = $this->m_report_fa_ntb->get_detail_data_by_id($params);
        if (empty($result)) {
            redirect('task/registration_fa');
        }
        // assign
        $this->smarty->assign("result", $result);
        $this->smarty->assign("result_rute", $this->m_report_fa_ntb->get_data_rute_by_id($params));
        $this->smarty->assign("total_rute", COUNT($this->m_report_fa_ntb->get_data_rute_by_id($params)));
        $this->smarty->assign("no", 1);
        // get remark field
        $this->smarty->assign("remark_field", $this->m_report_fa_ntb->get_remark_field(array($result['data_type'], $result['data_flight'], $result['services_cd'])));
        // output
        parent::display();
    }
    ...
    public function download() {
        ...
        // get list
        $params = array($tanggal_from, $tanggal_to, $tanggal_from, $tanggal_to, $published_no, $published_no, $data_type, $data_flight, $payment_st, $airlines_nm, $services_cd, 3, 4, 5, 6);
        $rs_id = $this->m_report_fa_ntb->get_all_data_report($params);
        ...
        // output file
        $file_name = 'REKAPITULASI_FLIGHT_APPROVAL_NIAGA_TIDAK_BERJADWAL';
        //--
    ...
    public function download_detail($data_id = "") {
        ...
        $params = array($data_id);
        $result = $this->m_report_fa_ntb->get_detail_data_by_id($params);
        $result_rute = $this->m_report_fa_ntb->get_data_rute_by_id(array($data_id));
        $total_rute = COUNT($result_rute);
        // get remark field
        $remark_field = $this->m_report_fa_ntb->get_remark_field(array($result['data_type'], $result['data_flight'], $result['services_cd']));
