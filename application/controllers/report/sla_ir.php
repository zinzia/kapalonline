...
class sla_ir extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "report/sla_ir/index.html");
        // load javascript
    ...
    public function download() {
        ...
        // get data
        // create excel
        $filepath = "resource/doc/template/TEMPLATE_REPORT_SLA_IR.xlsx";
        // ---
        ...
        // dicetak pada
        $objWorksheet->setCellValue('E4', strtoupper($this->datetimemanipulation->get_full_date(date('Y-m-d'))));
        ...
        foreach ($rs_id as $data) {
            ...
            $objWorksheet->setCellValue('D' . $i, $data['total_proses']);
            $objWorksheet->setCellValue('E' . $i, $data['response_time']);
            // style
        ...
        // output file
        $file_name = 'REKAPITULASI_REPORT_SLA_IZIN_RUTE';
        //--
    ...
    // chart jumlah fa
    public function chart_xml_jumlah_ir() {
        // bulan
        $bulan = array(
            '1' => 'Januari',
            '2' => 'Februari',
        ...
        foreach ($rs_id as $data) {
            $str_xml .= "<set label='" . strtoupper($data['operator_name']) . "' value='" . $data['total_proses'] . "'/>";
        }
        $str_xml .= "</chart>";
    ...
    // chart respon
    public function chart_xml_respon_ir($total = '') {
        // bulan
        $bulan = array(
            '1' => 'Januari',
            '2' => 'Februari',
        ...
        $str_xml = "<chart showPercentValues='1' showLabels='1' showValues='1' baseFontSize='8'>";
        foreach ($rs_id as $data) {
            $str_xml .= "<set label='" . strtoupper($data['operator_name']) . "' value='" . $data['detik'] . "'/>";
        }
        $str_xml .= "</chart>";
