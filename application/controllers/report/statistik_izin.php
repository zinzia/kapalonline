...
class statistik_izin extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('m_report_statistik_izin');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "report/statistik_izin/index.html");
        // load javascript
        ...
        $rs_id = $this->m_report_statistik_izin->get_data_statistik_pelayanan_izin($params);
        $data = array();
        $total = 0;
        $total_awaiting = 0;
        $total_approved = 0;
        ...
        foreach ($rs_id as $value) {
            $data[$value['izin_approval']][str_replace(' ', '_', $value['izin_flight'])][$value['izin_group']] = $value['total'];
            $total += $value['total'];
            // waiting
            if ($value['izin_approval'] == 'waiting') {
                $total_awaiting += $value['total'];
            }
            // approved
            if ($value['izin_approval'] == 'approved') {
                $total_approved += $value['total'];
            }
            // rejected
            if ($value['izin_approval'] == 'rejected') {
                $total_rejected += $value['total'];
            }
            // cancel
            if ($value['izin_approval'] == 'cancel') {
                $total_cancel += $value['total'];
            }
        }
        ...
        $this->smarty->assign("total_cancel", $total_cancel);
        // // belum diproses
        // // statistik
        // $rs_id = $this->m_report_statistik_izin->get_data_statistik_pelayanan_izin_belum_proses($params);
        // $data = array();
        // $total = 0;
        // foreach ($rs_id as $value) {
        //     $data[str_replace(' ', '_', $value['izin_flight'])][$value['izin_group']] = $value['total'];
        //     $total += $value['total'];
        // }
        // $this->smarty->assign("data_belum", $data);
        // $this->smarty->assign("total_belum", $total);
        // get list airlines
        $this->smarty->assign("rs_airlines", $this->m_report_statistik_izin->get_list_airlines());
        // output
        parent::display();
    }
    ...
    public function proses_cari() {
        ...
        // redirect
        redirect("report/statistik_izin");
    }
    ...
    public function data_chart() {
        ...
        // BERJADWAL
        $rs_id = $this->m_report_statistik_izin->get_data_chart_fa(array('berjadwal'));
        $data_berjadwal = array();
        foreach ($rs_id as $data) {
            $data_berjadwal[$data['bulan']] = $data['total'];
        }
        // TIDAK BERJADWAL
        $rs_id = $this->m_report_statistik_izin->get_data_chart_fa(array('tidak berjadwal'));
        $data_tidak_berjadwal = array();
        foreach ($rs_id as $data) {
            $data_tidak_berjadwal[$data['bulan']] = $data['total'];
        }
        // BUKAN NIAGA
        $rs_id = $this->m_report_statistik_izin->get_data_chart_fa(array('bukan niaga'));
    ...
    public function data_chart_pie() {
        ...
        // BUKAN NIAGA
        $rs_id = $this->m_report_statistik_izin->get_data_chart_pie($params);
        $data = array();
        foreach ($rs_id as $value) {
