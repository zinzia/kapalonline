...
class penghentian extends ApplicationBase {
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_pending_internasional/penghentian/index.html");
        // load javascript
    ...
    public function edit_process() {
        ...
        if ($action == 'Batalkan Permohonan') {
            ...
            if ($this->m_registrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_internasional/penghentian/index/" . $this->input->post('registrasi_id'));
            }
        }
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_registrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_internasional/penghentian/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_pending_internasional/penghentian/list.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_registrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
    ...
    public function list_rute_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            redirect("izin_pending_internasional/penghentian/list_files/" . $this->input->post('registrasi_id'));
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function list_files($registrasi_id = "") {
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "izin_pending_internasional/penghentian/files.html");
    ...
    public function files_process() {
        ...
        // default redirect
        redirect("izin_pending_internasional/penghentian/list_files/" . $registrasi_id);
    }
    ...
    public function files_download($data_id = "", $ref_id = "") {
        ...
        if (empty($result)) {
            redirect("izin_pending_internasional/penghentian/list_files/" . $data_id);
        }
        // filepath
    ...
    public function files_next() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // default redirect
            redirect("izin_pending_internasional/penghentian/review/" . $registrasi_id);
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function review($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_internasional/penghentian/review.html");
        // detail registrasi
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_registrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
