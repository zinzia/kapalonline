...
class migrasi extends ApplicationBase {
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_pending_domestik/migrasi/index.html");
        // load javascript
    ...
    public function edit_process() {
        ...
        if ($action == 'Batalkan Permohonan') {
            ...
            if ($this->m_migrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_domestik/migrasi/index/" . $this->input->post('registrasi_id'));
            }
        }
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_migrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_domestik/migrasi/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/migrasi/list.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_migrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
    ...
    public function list_rute_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // redirect
            redirect("izin_pending_domestik/migrasi/review/" . $this->input->post('registrasi_id'));
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function rute_add_process($registrasi_id = "") {
        ...
        if (empty($total)) {
            ...
            // redirect
            redirect('izin_pending_domestik/migrasi/list_rute/' . $registrasi_id);
        }
        // check bandara
        $is_used_score = 0;
        ...
        if ($this->m_migrasi->insert_izin_rute($params)) {
            ...
            redirect('izin_pending_domestik/migrasi/rute_data/' . $registrasi_id . '/' . $izin_id);
        } else {
            // notification
    ...
    public function rute_data($registrasi_id = "", $izin_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/migrasi/list_izin_data.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if (!empty($flight_no_from)) {
                if ($flight_no_from <> $this->input->post('flight_no')) {
                    ...
                    // redirect
                    redirect('izin_pending_domestik/migrasi/rute_data/' . $this->input->post('registrasi_id') . '/' . $this->input->post('izin_id'));
                }
            }
    ...
    public function review($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/migrasi/review.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_migrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
    ...
    public function send_process() {
        ...
        // default redirect
        redirect("izin_pending_domestik/migrasi/review/" . $registrasi_id);
    }
