...
class perubahan extends ApplicationBase {
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/index.html");
        // load javascript
    ...
    public function edit_process() {
        ...
        if ($action == 'Batalkan Permohonan') {
            ...
            if ($this->m_registrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_domestik/perubahan/index/" . $this->input->post('registrasi_id'));
            }
        }
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_registrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_domestik/perubahan/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/list.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_registrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
    ...
    public function list_rute_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($is_used_score <> 2) {
                redirect("izin_pending_domestik/perubahan/list_slot/" . $this->input->post('registrasi_id'));
            } else {
                // redirect
                redirect("izin_pending_domestik/perubahan/list_files/" . $this->input->post('registrasi_id'));
            }
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
    ...
    public function rute_add_new_process($registrasi_id = "") {
        ...
        if (empty($total)) {
            ...
            // redirect
            redirect('izin_pending_domestik/perubahan/list_rute/' . $registrasi_id);
        }
        // check bandara
        $is_used_score = 0;
        $airport = explode('-', $result['izin_rute_start']);
        ...
        if ($this->m_registrasi->insert_izin_rute($params)) {
            ...
            redirect('izin_pending_domestik/perubahan/rute_data/' . $registrasi_id . '/' . $izin_id);
        } else {
            // notification
    ...
    public function rute_data($registrasi_id = "", $izin_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/list_izin_data.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if (!empty($flight_no_from)) {
                if ($flight_no_from <> $this->input->post('flight_no')) {
                    ...
                    // redirect
                    redirect('izin_pending_domestik/perubahan/rute_data/' . $this->input->post('registrasi_id') . '/' . $this->input->post('izin_id'));
                }
            }
    ...
    public function list_slot($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/list_slot.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_registrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
        ...
        if ($is_used_score == 2) {
            // redirect
            redirect("izin_pending_domestik/perubahan/list_files/" . $registrasi_id);
        }
        // list slot
    ...
    public function slot_add($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/slot_add.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        // load style ui
        $this->smarty->load_style("jquery.ui/redmond/jquery-ui-1.8.13.custom.css");
    ...
    public function add_slot_process() {
        ...
        // default redirect
        redirect("izin_pending_domestik/perubahan/slot_add/" . $this->input->post('registrasi_id'));
    }
    ...
    public function slot_edit($registrasi_id = "", $slot_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/slot_edit.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        // load style ui
        $this->smarty->load_style("jquery.ui/redmond/jquery-ui-1.8.13.custom.css");
    ...
    public function edit_slot_process() {
        ...
        $result = $this->m_registrasi->get_detail_slot_by_id(array($registrasi_id, $slot_id));
        if (empty($result)) {
            redirect('izin_pending_domestik/perubahan/list_slot/' . $registrasi_id);
        }
        // process
        if ($this->tnotification->run() !== FALSE) {
        ...
        // default redirect
        redirect("izin_pending_domestik/perubahan/slot_edit/" . $registrasi_id . '/' . $slot_id);
    }
    ...
    public function list_files($registrasi_id = "") {
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/files.html");
        // get detail data registrasi
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_registrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
    ...
    public function files_download($data_id = "", $ref_id = "") {
        ...
        if (empty($result)) {
            redirect("izin_pending_domestik/perubahan/list_files/" . $data_id);
        }
        // filepath
    ...
    public function files_next() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // default redirect
            redirect("izin_pending_domestik/perubahan/review/" . $registrasi_id);
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function review($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_domestik/perubahan/review.html"); // get detail data
        // detail registrasi
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_registrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
