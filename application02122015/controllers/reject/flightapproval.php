...
class flightapproval extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('m_reject_regulator');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "reject/flightapproval/index.html");
        // load javascript
        ...
        $search = $this->tsession->userdata('regulator_search_reject_flightapproval');
        // search parameters
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("reject/flightapproval/index/");
        $config['total_rows'] = $this->m_reject_regulator->get_total_finished_task(array($tanggal_from, $tanggal_to, $tanggal_from, $tanggal_to, $published_no, $data_type, $data_flight));
        ...
        /* end of pagination ---------------------- */
        // get list
        $params = array($tanggal_from, $tanggal_to, $tanggal_from, $tanggal_to, $published_no, $data_type, $data_flight, ($start - 1), $config['per_page']);
        $rs_id = $this->m_reject_regulator->get_list_finished_task($params);
    ...
    public function proses_cari() {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('regulator_search_reject_flightapproval');
        } else {
            $params = array(
            ...
            $this->tsession->set_userdata("regulator_search_reject_flightapproval", $params);
        }
        // redirect
        redirect("reject/flightapproval");
    }
    ...
    public function print_out() {
        ...
        $data_flight = empty($search['data_flight']) ? '%' : '%' . $search['data_flight'] . '%';
        // get list
        $params = array($tanggal_from, $tanggal_to, $tanggal_from, $tanggal_to, $published_no, $data_type, $data_flight);
        $rs_id = $this->m_reject_regulator->get_list_finished_task_print($params);
        if (empty($rs_id)) {
            // default redirect
            redirect("reject/flightapproval");
        }
