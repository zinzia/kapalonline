...
class izinrute extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('reject/m_reject_regulator');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "reject/izinrute/index.html");
        // get tahun
        $this->smarty->assign("rs_tahun", $this->m_reject_regulator->get_list_tahun_report());
        // bulan
        $bulan = array(
            '01' => 'Januari',
            '02' => 'Februari',
        ...
        $search = $this->tsession->userdata('regulator_search_reject_izinrute');
        // search parameters
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("reject/izinrute/index/");
        $config['total_rows'] = $this->m_reject_regulator->get_total_finished_izin_registrasi(array($tahun, $bulan, $data_flight, $payment_st));
        ...
        /* end of pagination ---------------------- */
        // get list
        $params = array($tahun, $bulan, $data_flight, $payment_st, ($start - 1), $config['per_page']);
        $rs_id = $this->m_reject_regulator->get_list_finished_izin_registrasi($params);
    ...
    public function proses_cari() {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('regulator_search_reject_izinrute');
        } else {
            $params = array(
            ...
            $this->tsession->set_userdata("regulator_search_reject_izinrute", $params);
        }
        // redirect
        redirect("reject/izinrute");
    }
    ...
    public function baru($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/baru.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
        $detail = $this->m_reject_regulator->get_detail_registrasi_by_id(array($registrasi_id));
        //print_r($detail);
        if (empty($detail)) {
            redirect('reject/izinrute');
        }
        ...
        $notes = array();
        $data = array();
        $rs_id = $this->m_reject_regulator->get_list_izin_rute_reject_by_id(array($registrasi_id, $detail['airlines_id']));
        $no = 1;
        $total_rows = 0;
        foreach ($rs_id as $izin_rute) {
            $izin_data = $this->m_reject_regulator->get_list_izin_data_by_id(array($izin_rute['izin_id']));
            if (!empty($izin_data)) {
                // lebih dari 1 data
                $data[$no++] = $izin_data;
            } else {
        ...
        $this->smarty->assign("total_approved", $this->m_reject_regulator->get_total_frekuensi_reject_by_registrasi_id($registrasi_id));
        /*
        ...
        foreach ($kode_frekuensi as $kode => $st) {
            // get izin rute
            $izin_rute = $this->m_reject_regulator->get_history_izin_rute_old_by_kode_frekuensi(array($kode, $detail['airlines_id'], $detail['published_number'], $detail['registrasi_id'], $registrasi_id));
            // get izin data
            $izin_data = $this->m_reject_regulator->get_list_izin_data_by_id(array($izin_rute['izin_id']));
            if (!empty($izin_data)) {
                // lebih dari 1 data
        ...
        if (in_array($detail['izin_group'], $group)) {
            $an = 'DRJU-DAU';
            // get user dirjen 63
            $users = $this->m_reject_regulator->get_com_user_by_role(array(63));
        } else {
            $an = 'DJPU-DAU';
            // get user direktur 45
            $users = $this->m_reject_regulator->get_com_user_by_role(array(45));
        }
        ...
        $this->smarty->assign("terbilang", $this->m_reject_regulator->terbilang($lampiran));
        // get preferences
        $km = $this->m_reject_regulator->get_preferences_by_group_and_name(array('published_izin', 'KM'));
        ...
        foreach ($airport as $iata_code) {
            $data = $this->m_reject_regulator->get_airport_score_by_code(array($iata_code));
        ...
        $rs_slot = $this->m_reject_regulator->get_list_slot_time_by_id(array($registrasi_id, $detail['airlines_id']));
        $this->smarty->assign("rs_slot_non", $rs_slot);
        $this->smarty->assign("total_slot", count($rs_slot));
        // catatan tambahan
        $rs_memos = $this->m_reject_regulator->get_list_memos_by_izin(array($registrasi_id));
        $this->smarty->assign("rs_memos", $rs_memos);
        // redaksional
        $rs_redaksional = $this->m_reject_regulator->get_list_redaksional_by_registrasi(array($registrasi_id));
        $this->smarty->assign("rs_redaksional", $rs_redaksional);
        // kepada yang terpilih
        $rs_kepada = $this->m_reject_regulator->get_list_editorial_by_registrasi(array($registrasi_id, 'kepada'));
        $this->smarty->assign("rs_kepada", $rs_kepada);
        // tembusan yang terpilih
        $rs_tembusan = $this->m_reject_regulator->get_list_editorial_by_registrasi(array($registrasi_id, 'tembusan'));
    ...
    public function download_baru($registrasi_id = "") {
        ...
        $notes = array();
        $result = array();
        $rs_id = $this->m_reject_regulator->get_izin_data_by_registrasi_id(array($registrasi_id, $detail['airlines_id']));
        $no = 0;
        $last_key = "";
        foreach ($rs_id as $new) {
            ...
            $izin_rute = $this->m_reject_regulator->get_total_frekuensi(array($registrasi_id, $detail['airlines_id'], $new['izin_id']));
            // total frekuensi
        ...
        $terbilang = $this->m_reject_regulator->terbilang($total_lampiran);
        if ($detail["izin_flight"] == "domestik") {
            $nomor_surat = $this->m_reject_regulator->get_published_number_dom($an);
        } else {
            $nomor_surat = $this->m_reject_regulator->get_published_number_int($an);
        }
        // get preferences
        $km = $this->m_reject_regulator->get_preferences_by_group_and_name(array('published_izin', 'KM'));
        // list slot by IASM
        ...
        $rs_tembusan = $this->m_reject_regulator->get_list_editorial_by_registrasi(array($registrasi_id, 'tembusan'));
        // total approved
        $total_approved = $this->m_reject_regulator->get_total_frekuensi_reject_by_registrasi_id($registrasi_id);
        ...
        $barcode_url = $this->m_reject_regulator->get_barcode_value(array('baru'));
...
//         exit;
    ...
    public function perpanjangan($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/perpanjangan.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
        ...
        foreach ($kode_frekuensi as $kode => $st) {
            ...
            $surat_persetujuan = $this->m_reject_regulator->get_published_letter_old(array($izin_rute['registrasi_id'], $detail['airlines_id']));
        }
        $this->smarty->assign("rs_old", $data);
        $this->smarty->assign("frekuensi_old", $frekuensi_old);
        $this->smarty->assign("pairing_old", $pairing_old);
    ...
    public function download_perpanjangan($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_reject_regulator->get_barcode_value(array('perpanjangan'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
...
//         exit;
    ...
    public function penundaan($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/penundaan.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
        ...
        foreach ($kode_frekuensi as $kode => $st) {
            // get izin rute
            $izin_rute = $this->m_reject_regulator->get_history_izin_rute_old_by_kode_frekuensi(array($kode, $detail['airlines_id'], $detail['published_number'], $registrasi_id));
            // get izin data
            $izin_data = $this->m_reject_regulator->get_list_izin_data_by_id(array($izin_rute['izin_id']));
            if (!empty($izin_data)) {
                // lebih dari 1 data
    ...
    public function download_penundaan($registrasi_id = "") {
        ...
        foreach ($rs_old as $old) {
            ...
            // get total frekuensi and pairing
            $izin_rute = $this->m_reject_regulator->get_total_frekuensi(array($registrasi_id, $detail['airlines_id'], $old['izin_id']));
            // total frekuensi
        ...
        //set parameter barcode
        $barcode_url = $this->m_reject_regulator->get_barcode_value(array('penundaan'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
...
//         exit;
    ...
    public function penghentian($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/penghentian.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_penghentian($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_reject_regulator->get_barcode_value(array('penghentian'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
    ...
    public function frekuensi_add($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/frekuensi_add.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_frekuensi_add($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_reject_regulator->get_barcode_value(array('frekuensi_add'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
...
//         exit;
    ...
    public function frekuensi_delete($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/frekuensi_delete.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function perubahan($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/perubahan.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_perubahan($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_reject_regulator->get_barcode_value(array('perubahan'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
...
//         exit;
    ...
    public function frekuensi($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/frekuensi.html");
        // get detail data
        $params = array($registrasi_id);
        $result = $this->m_reject_regulator->get_rejected_izin_by_registrasi($params);
        if (empty($result)) {
            redirect('reject/izinrute');
        }
        $this->smarty->assign("result", $result);
        // get preferences
        $km = $this->m_reject_regulator->get_preferences_by_group_and_name(array('izinrute', 'KM'));
        $this->smarty->assign("km", $km);
        // list slot
        $rs_slot = $this->m_reject_regulator->get_list_data_slot_by_id(array($registrasi_id));
        $this->smarty->assign("rs_slot", $rs_slot);
        $this->smarty->assign("total_slot", count($rs_slot));
        // redaksional
        $rs_redaksional = $this->m_reject_regulator->get_list_redaksional(array($registrasi_id));
        $this->smarty->assign("rs_redaksional", $rs_redaksional);
        $this->smarty->assign("total_redaksional", count($rs_redaksional));
        // editorial kepada
        $rs_editorial_kepada = $this->m_reject_regulator->get_list_editorial_kepada(array($registrasi_id));
        $this->smarty->assign("rs_editorial_kepada", $rs_editorial_kepada);
        // editorial tembusan
        $rs_editorial_tembusan = $this->m_reject_regulator->get_list_editorial_tembusan(array($registrasi_id));
        $this->smarty->assign("rs_editorial_tembusan", $rs_editorial_tembusan);
        // get list frekuensi
        $params = array($registrasi_id);
        $rs_id = $this->m_reject_regulator->get_izin_rute_data_by_kode_izin($params);
        if (empty($rs_id)) {
            redirect('reject/izinrute');
        }
        $this->smarty->assign("rs_id", $rs_id);
        ...
        foreach ($rs_id as $new) {
            if ($temp <> $new['kode_frekuensi']) {
                $old = $this->m_reject_regulator->get_list_data_rute_by_kode_frekuensi_old(array($new['kode_frekuensi'], $registrasi_id, $new['published_number'], $new['published_number']));
        ...
        foreach ($rs_id as $data) {
            ...
            $result = $this->m_reject_regulator->get_surat_ijin_by_kode_frekuensi($params);
    ...
    public function download_frekuensi($registrasi_id = "") {
        ...
        // get preferences
        $km = $this->m_reject_regulator->get_preferences_by_group_and_name(array('reject_izin', 'KM'));
        // list slot
        $rs_slot = $this->m_reject_regulator->get_list_data_slot_by_id(array($registrasi_id, $this->com_user['airlines_id']));
        ...
        foreach ($rs_id as $new) {
            if ($temp <> $new['kode_frekuensi']) {
                $old = $this->m_reject_regulator->get_list_data_rute_by_kode_frekuensi_old(array($new['kode_frekuensi'], $registrasi_id, $this->com_user['airlines_id'], $new['reject_number']));
                $rs_old = array_merge($rs_old, $old);
            }
            $temp = $new['kode_frekuensi'];
        }
        ...
        foreach ($rs_id as $data) {
            ...
            $results = $this->m_reject_regulator->get_surat_ijin_by_kode_frekuensi($params);
                        ...
                        <table width="100%" cellpadding="2px">
                            ...
                            <table class="table-form" width="100%" cellpadding="1">
                                ...
                                <tr>
                                    <td style="vertical-align: top;">Perihal</td>
                                    <td style="vertical-align: top;">:</td>
                                    <td style="vertical-align: top;">Penolakan ' . $result["group_nm"];
        if ($result["izin_flight"] == "domestik") {
            $html .= '<br>Dalam Negeri';
        } else {
            $html .= '<br>Luar Negeri';
        }
        $html .= '
                                ...
                                <tr>
                                    ...
                                    <td colspan="3" style="text-align:justify; font-size:25px;">
                                        <ol>
                                            <li style="line-height:150%;">Menunjuk :
                                                <ol style="list-style-type: lower-alpha;">
                                                <li style="line-height:150%;">Peraturan Menteri Perhubungan Nomor ' . $km["pref_value"] . ';</li>
                                                    <li style="line-height:150%;">Peraturan Menteri Perhubungan Republik Indonesia Nomor : ' . $pm["pref_value"] . ' Tentang Perubahan Atas Peraturan Menteri Perhubungan Nomor ' . $km["pref_value"] . ';</li>';
        // foreach ($surat_persetujuan as $data) {
        ...
        // }
        $html .= '                              <li style="line-height:150%;">Surat dari ' . strtoupper($result["airlines_nm"]) . ' Nomor: ' . $result["izin_request_letter"] . ' Tanggal ' . $this->datetimemanipulation->get_full_date($result["izin_request_letter_date"]) . '
        ...
        $html.= '
                                                ...
                                                <br />
                                                dengan hormat disampaikan bahwa Direktorat Jenderal Perhubungan Udara <b> <i>belum dapat memberikan izin </i></b> pengurangan rute penerbangan';
        ...
        for ($i = 2; $i <= $total_lampiran; $i++) {
            $html .= '
                                    ...
                                    <hr>
                                    <p class="caption">
                                        <b>PENOLAKAN ' . strtoupper($result["group_nm"]) . '</b>
                                        <br />
                                    ...
                                    <table class="table-view" width="100%" style="font-size: 20px; font-family: times; color: #333;" border="1" cellpadding="2">
                                        <tr>
                                            ...
                                            <td width="10%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Hari<br /> Operasi</td>
                                            <td width="17%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Periode</td>
                                        </tr>
                                        <tr>
            ...
            for ($x = $izin; $x < $per_page; $x++) {
                if ($rs_id[$x]["izin_id"] != "") {
                    ...
                    $html .= '<tr>';
                    if ($rs_old[$y]["rute_all"] != $rs_id[$x]["rute_all"]) {
                        $html .= '<td align="center"> ' . $rs_id[$y]["rute_all"] . '</td>';
                    }
        ...
        $this->tcpdf->writeHTML($html, true, false, true, false, '');
        // output (D : download, I : view)
        $filename = str_replace(" ", "_", "PENOLAKAN_" . strtoupper($result["group_nm"])) . "_" . str_replace(" ", "_", strtoupper($result["airlines_nm"]));
    ...
    public function notam($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/notam.html");
        // get detail data
        $params = array($registrasi_id);
        $result = $this->m_reject_regulator->get_rejected_izin_by_registrasi($params);
        if (empty($result)) {
        ...
        foreach ($rs_id as $new) {
            if ($temp <> $new['kode_frekuensi']) {
                $old = $this->m_reject_regulator->get_list_data_rute_by_kode_frekuensi_old(array($new['kode_frekuensi'], $new['published_number']));
                $rs_old = array_merge($rs_old, $old);
            }
            $temp = $new['kode_frekuensi'];
        }
    ...
    public function download_notam($registrasi_id = "") {
        ...
        foreach ($rs_id as $new) {
            if ($temp <> $new['kode_frekuensi']) {
                $old = $this->m_reject_regulator->get_list_data_rute_by_kode_frekuensi_old(array($new['kode_frekuensi'], $registrasi_id, $this->com_user, $new['reject_number']));
                $rs_old = array_merge($rs_old, $old);
            }
            $temp = $new['kode_frekuensi'];
        }
                                        ...
                                        <ol>
                                            <li style="line-height:150%;">Menunjuk :
                                                <ol style="list-style-type: lower-alpha;">
                                                    <li style="line-height:150%;">Peraturan Menteri Perhubungan Nomor ' . $km["pref_value"] . ';</li>
                                                    <li style="line-height:150%;">Peraturan Menteri Perhubungan Republik Indonesia Nomor : ' . $pm["pref_value"] . ' Tentang Perubahan Atas Peraturan Menteri Perhubungan Nomor ' . $km["pref_value"] . ';</li>';
//        $html .='                                   <li style="line-height:150%;">Peraturan Menteri Perhubungan Nomor ' . $km["pref_value"] . ' dan peraturan perundang - undangan yang terkait dengan penerbangan;
//                                                    </li>';
//        foreach ($surat_persetujuan as $data) {
//            $html .= '<li style="line-height:150%;">Surat Direktorat Jenderal Perhubungan Udara Nomor: ' . $data["nomor"] . ' tanggal ' . $this->datetimemanipulation->get_full_date($data["tanggal"]) . ';</li>;';
//        }
        ...
        $html.= '<br />
            dengan hormat disampaikan bahwa Direktorat Jenderal Perhubungan Udara <b> <i>belum dapat memberikan izin </i></b> rute penerbangan';
        if ($result["pax_cargo"] == "cargo") {
            $html .= ' cargo';
        }
        $html .= '  berjadwal kepada ' . strtoupper($result["airlines_nm"]) . ', sesuai dengan jadwal penerbangan sebagaimana terlampir. Dengan pertimbangan bahwa : </br>
<ol style="margin: 0 0 0 20px; padding: 0; line-height: 24px; list-style-type: lower-alpha;">';
        foreach ($rs_memos as $data) {
            $html .= '  <li style="line-height:150%;">' . $data["memo"] . ';</li>';
        }
...
//            $html .= '<li style="line-height:150%;">' . $data["memo"] . ';</li>';
//        }
//        $html .= '<li style="line-height:150%;">Dimohon kepada ' . $redaksional . ' untuk meneruskan surat persetujuan ini kepada jajaran di lingkungan wilayah kerjanya untuk dilakukan pengawasan dalam pelaksanaannya.</li>';
        $html .= '<li style="line-height:150%;">Demikian disampaikan, atas perhatiannya diucapkan terima kasih.</li>
                                        </ol>
                                    </td>
                                </tr>
    ...
    public function migrasi($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "reject/izinrute/migrasi.html");
        // get detail data
        $params = array($registrasi_id);
        $result = $this->m_reject_regulator->get_rejected_izin_by_registrasi($params);
        if (empty($result)) {
                        ...
                        <div class="content">
                                    ...
                                    <table class="table-view" width="100%" style="font-size: 20px; font-family: times; color: #333;" border="1" cellpadding="2">
                                        <tr>
                                            <td width="5%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'No</td>
                                            <td width="15%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Rute<br />Penerbangan</td>
                                            <td width="15%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Tipe<br />Pesawat</td>
                                            <td width="15%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Nomor<br />Penerbangan</td>
                                            <td width="20%" colspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Jadwal Penerbangan <br />(Waktu Lokal)</td>
                                            <td width="15%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Frek</td>
                                            <td width="15%" rowspan="2" align="center">' . str_repeat('&nbsp;<br/>', 1) . 'Hari<br /> Operasi</td>
                                        </tr>
                                        <tr>
                                            <td width="10%" align="center">ETD</td>
                                            <td width="10%" align="center">ETA</td>
            ...
            if ($rs_id[$x]["izin_id"] != "") {
                ...
                if ($rs_id[$x]["izin_id"] != $temp) {
                    if ($rs_id[$x]["pairing"] == "VV") {
                        $html .= '<td align="center" rowspan="2"> ' . (str_repeat('&nbsp;<br/>', $rs_id[$x]["rowspan"] - 1)) . $no . '.</td>';
                    } else {
                        $html .= '<td align="center"> ' . (str_repeat('&nbsp;<br/>', $rs_id[$x]["rowspan"] - 1)) . $no . '.</td>';
                    }
                }
                $html .= '
                                                <td align="center">' . $rs_id[$x]["rute_all"] . '</td>
