...
class registration_domestik extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "member/registration_domestik/index.html");
        // list group
        $rs_id = $this->m_izin->get_list_group_by_flight('domestik');
        $this->smarty->assign("rs_group", $rs_id);
        // list opened form
        $rs_id = $this->m_izin->get_list_registration_open(array('domestik', $this->com_user['airlines_id']));
    ...
    public function delete_process($data_id = "") {
        ...
        // execute
        if ($this->m_izin->delete_izin($params)) {
            // delete izin rute
            $this->m_izin->delete_rute_by_registrasi($data_id);
            // success
    ...
    public function disclaimer($group_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/registration_domestik/disclaimer.html");
        // detail group
    ...
    public function disclaimer_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->session->userdata('word') == $captcha AND $this->session->userdata('ip_address') == $_SERVER["REMOTE_ADDR"] AND $this->session->userdata('captcha_time') > $expiration) {
                ...
                // redirect
                redirect("member/registration_domestik/create/" . $group_id);
            } else {
        ...
        // redirect
        redirect("member/registration_domestik/disclaimer/" . $group_id);
    }
    ...
    public function create($group_id = "") {
        ...
        $id = $this->m_izin->create_group_data($params);
        if (!empty($id)) {
            // notification
