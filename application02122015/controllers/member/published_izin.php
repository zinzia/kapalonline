...
class published_izin extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model("penerbitan/m_published_izin");
        $this->load->model('m_published_izin_regulator');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "member/published_izin/index.html");
        // get tahun
        $this->smarty->assign("rs_tahun", $this->m_published_izin->get_list_tahun_report($this->com_user['airlines_id']));
        // bulan
        $bulan = array(
            '01' => 'Januari',
            '02' => 'Februari',
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("member/published_izin/index/");
        $config['total_rows'] = $this->m_published_izin->get_total_finished_izin_registrasi(array($this->com_user['airlines_id'], $tahun, $bulan, $data_flight, $payment_st));
        ...
        /* end of pagination ---------------------- */
        // get list
        $params = array($this->com_user['airlines_id'], $tahun, $bulan, $data_flight, $payment_st, ($start - 1), $config['per_page']);
        $rs_id = $this->m_published_izin->get_list_finished_izin_registrasi_by_airlines($params);
    ...
    public function baru($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/baru.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
        $detail = $this->m_published_izin->get_detail_registrasi_by_id(array($registrasi_id));
        if (empty($detail)) {
            redirect('member/published_izin');
        }
        ...
        $notes = array();
        $data = array();
        $rs_id = $this->m_published_izin->get_list_izin_rute_approved_by_id(array($registrasi_id, $detail['airlines_id']));
        $no = 1;
        $total_rows = 0;
        foreach ($rs_id as $izin_rute) {
            $izin_data = $this->m_published_izin->get_list_izin_data_by_id(array($izin_rute['izin_id']));
            if (!empty($izin_data)) {
                // lebih dari 1 data
                $data[$no++] = $izin_data;
            } else {
        ...
        $this->smarty->assign("total_approved", $this->m_published_izin->get_total_frekuensi_approved_by_registrasi_id($registrasi_id));
        /*
        ...
        foreach ($kode_frekuensi as $kode => $st) {
            // get izin rute
            $izin_rute = $this->m_published_izin->get_list_izin_rute_old_by_kode_frekuensi(array($st, $detail['airlines_id']));
            // get izin data
            $izin_data = $this->m_published_izin->get_list_izin_data_by_id(array($izin_rute['izin_id']));
            if (!empty($izin_data)) {
                // lebih dari 1 data
            ...
            // $surat_persetujuan = $this->m_published_izin->get_published_letter_old(array($izin_rute['registrasi_id'], $detail['airlines_id']));
        }
        ...
        if (in_array($detail['izin_group'], $group)) {
            $an = 'DRJU-DAU';
            // get user dirjen 63
            $users = $this->m_published_izin->get_com_user_by_role(array(63));
        } else {
            $an = 'DJPU-DAU';
            // get user direktur 45
            $users = $this->m_published_izin->get_com_user_by_role(array(45));
        }
        ...
        $this->smarty->assign("terbilang", $this->m_published_izin->terbilang($lampiran));
        // get preferences
        $km = $this->m_published_izin->get_preferences_by_group_and_name(array('published_izin', 'KM'));
        ...
        foreach ($airport as $iata_code) {
            $data = $this->m_published_izin->get_airport_score_by_code(array($iata_code));
        ...
        $rs_slot = $this->m_published_izin->get_list_slot_time_by_id(array($registrasi_id, $detail['airlines_id']));
        $this->smarty->assign("rs_slot_non", $rs_slot);
        $this->smarty->assign("total_slot", count($rs_slot));
        // catatan tambahan
        $rs_memos = $this->m_published_izin->get_list_memos_by_izin(array($registrasi_id));
        $this->smarty->assign("rs_memos", $rs_memos);
        // redaksional
        $rs_redaksional = $this->m_published_izin->get_list_redaksional_by_registrasi(array($registrasi_id));
        $this->smarty->assign("rs_redaksional", $rs_redaksional);
        // kepada yang terpilih
        $rs_kepada = $this->m_published_izin->get_list_editorial_by_registrasi(array($registrasi_id, 'kepada'));
        $this->smarty->assign("rs_kepada", $rs_kepada);
        // tembusan yang terpilih
        $rs_tembusan = $this->m_published_izin->get_list_editorial_by_registrasi(array($registrasi_id, 'tembusan'));
    ...
    public function download_baru($registrasi_id = "") {
        ...
        $notes = array();
        $result = array();
        $rs_id = $this->m_published_izin->get_izin_data_by_registrasi_id(array($registrasi_id, $detail['airlines_id']));
        $no = 0;
        $last_key = "";
        foreach ($rs_id as $new) {
            ...
            $izin_rute = $this->m_published_izin->get_total_frekuensi(array($registrasi_id, $detail['airlines_id'], $new['izin_id']));
            // total frekuensi
        ...
        $terbilang = $this->m_published_izin->terbilang($total_lampiran);
        if ($detail["izin_flight"] == "domestik") {
            $nomor_surat = $this->m_published_izin->get_published_number_dom($an);
        } else {
            $nomor_surat = $this->m_published_izin->get_published_number_int($an);
        }
        // get preferences
        $km = $this->m_published_izin->get_preferences_by_group_and_name(array('published_izin', 'KM'));       
        // list slot by IASM
        ...
        $rs_tembusan = $this->m_published_izin->get_list_editorial_by_registrasi(array($registrasi_id, 'tembusan'));
        // total approved
        $total_approved = $this->m_published_izin->get_total_frekuensi_approved_by_registrasi_id($registrasi_id);
        ...
        $barcode_url = $this->m_published_izin->get_barcode_value(array('baru'));
    ...
    public function perpanjangan($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/perpanjangan.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_perpanjangan($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_published_izin->get_barcode_value(array('perpanjangan'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
    ...
    public function penundaan($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/penundaan.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_penundaan($registrasi_id = "") {
        ...
        foreach ($rs_old as $old) {
            ...
            // get total frekuensi and pairing
            $izin_rute = $this->m_published_izin->get_total_frekuensi(array($registrasi_id, $detail['airlines_id'], $old['izin_id']));
            // total frekuensi
        ...
        //set parameter barcode
        $barcode_url = $this->m_published_izin->get_barcode_value(array('penundaan'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
    ...
    public function penghentian($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/penghentian.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_penghentian($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_published_izin->get_barcode_value(array('penghentian'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
...
//        }
    ...
    public function frekuensi_add($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/frekuensi_add.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_frekuensi_add($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_published_izin->get_barcode_value(array('frekuensi_add'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
    ...
    public function frekuensi_delete($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/frekuensi_delete.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function perubahan($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/perubahan.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
        ...
        $this->smarty->assign("notes", $notes);
        // get total frekuensi disetujui
        $this->smarty->assign("total_approved", $this->m_published_izin->get_total_frekuensi_approved_perubahan_by_registrasi_id($registrasi_id));
        /*
         * RUTE PENERBANGAN SEBELUMNYA
         */
        $no = 1;
    ...
    public function download_perubahan($registrasi_id = "") {
        ...
        $rs_tembusan = $this->m_published_izin->get_list_editorial_by_registrasi(array($registrasi_id, 'tembusan'));
        // total approved
        $total_approved = $this->m_published_izin->get_total_frekuensi_approved_perubahan_by_registrasi_id($registrasi_id);
        ...
        //set parameter barcode
        $barcode_url = $this->m_published_izin->get_barcode_value(array('perubahan'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
...
//         exit;
    ...
    public function frekuensi($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/frekuensi.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function download_frekuensi($registrasi_id = "") {
        ...
        //set parameter barcode
        $barcode_url = $this->m_published_izin->get_barcode_value(array('frekuensi'));
        $url = base_url() . $barcode_url['pref_value'] . $detail['registrasi_id'];
        $params_barcode = $this->tcpdf->serializeTCPDFtagParameters(array($url, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
        // unset tcpdf
        unset($this->tcpdf);
    ...
    public function notam($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/notam.html");
        /*
         * REGISTRASI
         */
        // detail registrasi
    ...
    public function migrasi($registrasi_id = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/published_izin/migrasi.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id']);
        $result = $this->m_published_izin->get_published_izin_by_registrasi($params);
        if (empty($result)) {
            redirect('member/published_izin');
        }
        ...
        // get list frekuensi
        $params = array($registrasi_id, $this->com_user['airlines_id']);
        $rs_id = $this->m_published_izin->get_izin_rute_data_by_kode_izin($params);
        if (empty($rs_id)) {
            redirect('member/published_izin');
        }
        ...
        //total frek
        $this->smarty->assign("tot_frek", $this->m_published_izin->get_total_frekuensi_by_registrasi_id($registrasi_id));
        // output
        parent::display();
    }
    ...
    public function download_migrasi($registrasi_id = "") {
        ...
        $tot_frek = $this->m_published_izin->get_total_frekuensi_by_registrasi_id($registrasi_id);        
        // get list frekuensi
        $params = array($registrasi_id, $this->com_user['airlines_id']);
        $rs_id = $this->m_published_izin->get_izin_rute_data_by_kode_izin($params);
        if (empty($rs_id)) {
        ...
        for ($x = $izin; $x < $total_data; $x++) {
            if ($rs_id[$x]["izin_id"] != "") {
                ...
                $html .= '
                                                <td align="center">' . $rs_id[$x]["rute_all"] . '</td>
                                                <td align="center">' . $rs_id[$x]["tipe"] . '</td>
                                                <td align="center">' . $rs_id[$x]["airlines_iata_cd"] . $rs_id[$x]["flight_no"] . '</td>
                                                <td align="center">' . substr($rs_id[$x]["etd"], 0, 5) . '</td>
                                                <td align="center">' . substr($rs_id[$x]["eta"], 0, 5) . '</td> 
                                                <td align="center">' . $rs_id[$x]["frekuensi"] . 'X</td>
                                                <td align="center">' . $rs_id[$x]["doop"] . '</td>
