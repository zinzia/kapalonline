...
class account_settings extends ApplicationBase {
    ...
    public function data_pribadi() {
        ...
        $this->smarty->assign("template_content", "member/account_settings/index.html");
        // load javascript
        ...
        // airlines
        $rs_id = $this->m_operator->get_list_airlines_by_user($this->com_user['user_id']);
        $this->smarty->assign("rs_airlines", $rs_id);
        // notification
    ...
    public function process_data_pribadi() {
        ...
        $this->tnotification->set_rules('member_status', 'Mmeber Status', 'trim|required');
        // process
        ...
        // default redirect
        redirect("member/account_settings/data_pribadi");
    }
    ...
    public function user_account() {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/account_settings/account.html");
        // get data pribadi
    ...
    public function process_update_account() {
        ...
        // default redirect
        redirect("member/account_settings/user_account");
    }
    ...
    public function change_role() {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "member/account_settings/role.html");
        // get data role
    ...
    public function process_update_role() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            // get user login
            $session = $this->tsession->userdata('session_operator_fa_manual');
            $this->tsession->set_userdata('session_operator_fa_manual', array('user_id' => $session['user_id'], 'role_id' => $this->input->post('role_id')));
            // notification
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
        } else {
        ...
        // default redirect
        redirect("member/account_settings/change_role");
    }

    // process update airlines
    public function process_change_airlines($airlines_id = "") {
        // cek input
        $result = $this->m_operator->check_airlines_user(array($this->com_user['user_id'], $airlines_id));
        // process
        if ($result) {
            // change session
            $session = $this->tsession->userdata('session_fa_member');
            $this->tsession->set_userdata('session_fa_member', array('user_id' => $session['user_id'], 'role_id' => $session['role_id'],
                'default_page' => $session['default_page'], 'airlines_id' => $airlines_id));
            // notification
            $this->tnotification->delete_last_field();
