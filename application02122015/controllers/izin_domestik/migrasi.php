...
class migrasi extends ApplicationBase {

    private $group_id = 10;
    private $season_cd = 'S15';
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load models
        $this->load->model('pendaftaran/m_migrasi');
        ...
        // get global season code
        $result = $this->m_preferences->get_preferences_by_group_and_name(array('season_code', 'aktif'));
        if (!empty($result)) {
            $this->season_cd = $result['pref_value'];
        }
    }
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_domestik/migrasi/index.html");
        // load javascript
    ...
    public function edit_process() {
        ...
        $this->tnotification->set_rules('izin_rute_start', 'Rute Pairing', 'trim|required|maxlength[7]');
        $this->tnotification->set_rules('izin_rute_end', 'Rute Pairing', 'trim|required|maxlength[7]');
        $this->tnotification->set_rules('pax_cargo', 'Jenis Penerbangan', 'trim|required');
        $this->tnotification->set_rules('izin_published_date', 'Tanggal Surat Diterbitkan', 'trim|required');
        $this->tnotification->set_rules('izin_published_letter', 'Nomor Surat Terbit', 'trim|required|maxlength[100]');
        // Validasi Pengajuan khusus migrasi, rute sudah pernah terdaftar
        $params = array(
            $this->com_user['airlines_id'], $this->input->post('registrasi_id'),
            $this->input->post('izin_rute_start'), $this->input->post('izin_rute_start'),
            $this->input->post('izin_rute_end'), $this->input->post('izin_rute_end'),
        );
        $total = $this->m_migrasi->get_total_rute_existing_by_migrasi($params);
        if ($total > 0) {
            $this->tnotification->set_error_message('Rute sudah pernah di migrasi sebelumya!');
        }
        // validasi jika rute sudah / dalam posisi akhir dicabut
        $total = $this->m_migrasi->get_total_rute_existing_canceled($params);
        if ($total > 0) {
        ...
        $total = $this->m_migrasi->get_total_rute_process_by_new_rute($params);
        if ($total > 0) {
        ...
        $params = array(
            $this->input->post('registrasi_id'),
            $this->input->post('izin_rute_start'),
            $this->input->post('izin_rute_end'),
        );
        $total = $this->m_migrasi->get_total_rute_by_registrasi_id($params);
        if ($total > 0) {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $params = array(
                ...
                "pax_cargo" => $this->input->post('pax_cargo'),
                "izin_published_date" => $this->input->post('izin_published_date'),
                "izin_published_letter" => $this->input->post('izin_published_letter'),
                "izin_request_letter" => '',
                "izin_season" => $this->season_cd,
            );
            // where
            $where = array(
            ...
            // insert
            if ($this->m_migrasi->update_izin_registrasi($params, $where)) {
                // unset session
                ...
                // default redirect
                redirect("izin_domestik/migrasi/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
        ...
        // default redirect
        redirect("izin_domestik/migrasi/index/" . $this->input->post('registrasi_id'));
    }
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_domestik/migrasi/list.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_migrasi->get_registrasi_waiting_by_id($params);
        if (empty($result)) {
        ...
        $frekuensi = array();
        $data = array();
        $rs_id = $this->m_migrasi->get_list_izin_rute_waiting_by_id(array($registrasi_id, $this->com_user['airlines_id']));
        $no = 1;
        foreach ($rs_id as $izin_rute) {
            $izin_data = $this->m_migrasi->get_list_izin_data_waiting_by_id(array($izin_rute['izin_id']));
            if (!empty($izin_data)) {
                $data[$no++] = $izin_data;
            } else {
        ...
        // frekuensi
        $this->smarty->assign("total", $this->m_migrasi->get_total_frekuensi_by_registrasi_id($registrasi_id));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function list_rute_process() {
        ...
        $total = $this->m_migrasi->get_total_izin_data_by_registrasi($params);
        if (empty($total)) {
            $this->tnotification->set_error_message('Data rute penerbangan belum diinput!');
        }
        // check jika msh ada yang kosong
        $params = array($this->input->post('registrasi_id'), $this->com_user['airlines_id']);
        $total = $this->m_migrasi->get_izin_rute_empty_data_by_id($params);
        if (empty($total)) {
            // notification
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // redirect
            redirect("izin_domestik/migrasi/review/" . $this->input->post('registrasi_id'));
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    // create rute id
    public function rute_add_process($registrasi_id = "") {
        // set page rules
        $this->_set_page_rule("C");
        // check registrasi
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        ...
        if (empty($total)) {
            ...
            // redirect
            redirect('izin_domestik/migrasi/list_rute/' . $registrasi_id);
        }
        // check bandara
        $is_used_score = 0;
        $airport = explode('-', $result['izin_rute_start']);
        foreach ($airport as $iata_code) {
            $data = $this->m_migrasi->get_airport_score_by_code(array($iata_code));
            $is_used_score += $data['is_used_score'];
        }
        // add izin rute
        $izin_id = $this->m_migrasi->get_data_id();
        $params = array(
            "izin_id" => $izin_id,
            "airlines_id" => $this->com_user['airlines_id'],
        ...
        if ($this->m_migrasi->insert_izin_rute($params)) {
            // notification
            $this->tnotification->sent_notification("success", "Data has been created!");
            redirect('izin_domestik/migrasi/rute_data/' . $registrasi_id . '/' . $izin_id);
        } else {
            // notification
    ...
    public function rute_delete($registrasi_id = "", $izin_id = "") {
        ...
        // execute
        if ($this->m_migrasi->delete_izin_rute($params)) {
            // success
            $this->tnotification->delete_last_field();
    ...
    public function rute_data($registrasi_id = "", $izin_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_domestik/migrasi/list_izin_data.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        ...
        $this->smarty->assign("detail", $result);
        // default season
        $this->season_cd = $this->m_migrasi->get_default_season();
        ...
        $rute = $this->m_migrasi->get_izin_rute_waiting_by_id($params);
        if (empty($rute)) {
            redirect('izin_domestik/migrasi/list_rute/' . $registrasi_id);
        }
        ...
        // list izin data
        $rs_id = $this->m_migrasi->get_list_izin_data_waiting_by_id(array($izin_id));
        $this->smarty->assign("rs_id", $rs_id);
        // frekuensi
        $this->smarty->assign("total", $this->m_migrasi->get_total_frekuensi_by_izin_id($izin_id));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // untuk dos selected
    ...
    public function rute_add_process_non_iasm() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $flight_no_from = $this->m_migrasi->get_flight_no_by_izin_id(array($this->input->post('izin_id'), $rute_from . '-' . $rute_to));
            if (!empty($flight_no_from)) {
                if ($flight_no_from <> $this->input->post('flight_no')) {
                    ...
                    // redirect
                    redirect('izin_domestik/migrasi/rute_data/' . $this->input->post('registrasi_id') . '/' . $this->input->post('izin_id'));
                }
            }

            // validasi untuk rute sebaliknya, nomor penerbangan juga tidak boleh sama
            $flight_no_to = $this->m_migrasi->get_flight_no_by_izin_id(array($this->input->post('izin_id'), $rute_to . '-' . $rute_from));
            ...
            $rute_id = $this->m_migrasi->get_data_id();
            $params = array(
                'rute_id' => $rute_id,
            ...
            // insert
            if ($this->m_migrasi->insert_izin_data($params)) {
                // get periode & frekuensi
                $frekuensi = $this->m_migrasi->get_total_frekuensi_by_izin_id(array($this->input->post('izin_id')));
                // update status layanan penerbangan OQ / VV
                $total = $this->m_migrasi->get_services_flight(array($this->input->post('izin_id')));
                if ($total == 2) {
                    // VV
                    $params = array(
                        'pairing' => 'VV',
                ...
                $this->m_migrasi->update_izin_rute($params, $where);
                // notification
                $this->tnotification->delete_last_field();
                $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            } else {
    ...
    public function rute_add_process_mixed() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_migrasi->insert_izin_data($params)) {
                ...
                $this->m_migrasi->delete_izin_data_slot($where);
                // insert slot
                $slot_id = $this->m_migrasi->get_data_id() + 1;
                ...
                $this->m_migrasi->insert_izin_data_slot($params);
                // notification
                $this->tnotification->delete_last_field();
                $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            } else {
    ...
    public function izin_data_delete_process($registrasi_id = "", $izin_id = "", $rute_id = "") {
        ...
        // execute
        if ($this->m_migrasi->delete_izin_data($params)) {
            // update status layanan penerbangan OQ / VV
            $total = $this->m_migrasi->get_services_flight(array($izin_id));
            if ($total == 2) {
                // VV
                $params = array(
                    'pairing' => 'VV',
    ...
    // STEP 3 : REVIEW PERMOHONAN
    public function review($registrasi_id = "") {
        // set rules
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_domestik/migrasi/review.html");
        //load helper
    ...
    public function send_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            // process flow
            $process_id = $this->m_migrasi->get_data_id();
            $params = array(
            ...
            $this->m_migrasi->insert_izin_process($params);
            // update status registrasi
            $total = $this->m_migrasi->get_total_frekuensi_by_registrasi_id($registrasi_id);
            // params
            $params = array(
                'izin_request_st' => '1',
                'izin_request_by' => $this->com_user['user_id'],
                'izin_request_date' => date('Y-m-d H:i:s'),
                'izin_valid_start' => $total['start_date'],
                'izin_valid_end' => $total['end_date'],
                'izin_valid_st' => 'no',
            );
            $where = array(
                'registrasi_id' => $registrasi_id,
            ...
            $this->m_migrasi->update_izin_registrasi($params, $where);
            // notification
            $this->tnotification->delete_last_field();
        ...
        // default redirect
        redirect("izin_domestik/migrasi/review/" . $registrasi_id);
    }
