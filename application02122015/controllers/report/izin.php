...
class izin extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "report/izin/index.html");
        // load javascript
        ...
        $search = $this->tsession->userdata('search_published_regulator');
        // search parameters
        ...
        // params
        $params = array($airlines_nm, $data_flight, $season_cd, $tanggal, $airport_iata_cd, $airport_iata_cd);
        // list
        $rs_id = $this->m_report_izin->get_izin_rute_data_by_airlines_regulator($params);
    ...
    public function detail($izin_rute_start = "", $izin_rute_end = "") {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "report/izin/detail.html");
        ...
        $this->smarty->assign("airlines_nm", $izin_data[0]["airlines_nm"]);
    ...
    public function proses_cari() {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('search_published_regulator');
        } else {
            $params = array(
            ...
            $this->tsession->set_userdata("search_published_regulator", $params);
        }
        // redirect
        redirect("report/izin");
    }
    ...
    public function download($izin_rute_start = "", $izin_rute_end = "") {
         ...
         * HTML
         */
        $html = '<p style="font-size: 24px;"><b>AIRLINES : ' . $izin_data[0]["airlines_nm"] . '</b><br>';        // content
        $html .= '<b>PERIODE &nbsp;: ' . $season_cd . ' / ' . strtoupper($bulans[$bulan]) . ' ' . $tahun . '</b></p>';        // content
        ...
        foreach ($data as $no => $izin_rute) {
            ...
            foreach ($izin_rute as $izin_data) {
                ...
                if (strlen(trim($izin_data['flight_no'])) > 4) {
                    $html .= '<td align="center">' . $izin_data['flight_no'] . '</td>';
                } else {
                    $html .= '<td align="center">' . $izin_data['airlines_iata_cd'] . $izin_data['flight_no'] . '</td>';
                }
