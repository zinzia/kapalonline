...
class migrasi extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('revisi/m_migrasi');
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_pending_internasional/migrasi/index.html");
        // load javascript
        ...
        $result = $this->m_migrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
        ...
        // catatan perbaikan
        $this->smarty->assign("catatan", $this->m_migrasi->get_catatan_perbaikan_by_registrasi($registrasi_id));
        // notification
    ...
    public function edit_process() {
        ...
        $this->tnotification->set_rules('izin_published_letter', 'Nomor Surat Terbit', 'trim|required|maxlength[50]');
        // get detail data
        ...
        if ($action == 'Batalkan Permohonan') {
            ...
            if ($this->m_migrasi->update_izin_registrasi($params, $where)) {
                ...
                $this->m_migrasi->update_izin_process($params, $where);
                // update izin rute
                $params = array(
                ...
                // default redirect
                redirect("izin_pending_internasional/migrasi/index/" . $this->input->post('registrasi_id'));
            }
        }
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_migrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_pending_internasional/migrasi/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_pending_internasional/migrasi/list.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_migrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
        ...
        $frekuensi = array();
        $data = array();
        $rs_id = $this->m_migrasi->get_list_izin_rute_pending_by_id(array($registrasi_id, $this->com_user['airlines_id']));
        $no = 1;
        foreach ($rs_id as $izin_rute) {
            $izin_data = $this->m_migrasi->get_list_izin_data_pending_by_id(array($izin_rute['izin_id']));
            if (!empty($izin_data)) {
                $data[$no++] = $izin_data;
            } else {
    ...
    public function list_rute_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // redirect
            redirect("izin_pending_internasional/migrasi/review/" . $this->input->post('registrasi_id'));
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function rute_add_process($registrasi_id = "") {
        ...
        if (empty($total)) {
            ...
            // redirect
            redirect('izin_pending_internasional/migrasi/list_rute/' . $registrasi_id);
        }
        // check bandara
        $is_used_score = 0;
        ...
        if ($this->m_migrasi->insert_izin_rute($params)) {
            ...
            redirect('izin_pending_internasional/migrasi/rute_data/' . $registrasi_id . '/' . $izin_id);
        } else {
            // notification
    ...
    public function rute_data($registrasi_id = "", $izin_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_internasional/migrasi/list_izin_data.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        ...
        $rute = $this->m_migrasi->get_izin_rute_pending_by_id($params);
        if (empty($rute)) {
            redirect('izin_pending_internasional/migrasi/list_rute/' . $registrasi_id);
        }
        ...
        // list izin data
        $rs_id = $this->m_migrasi->get_list_izin_data_pending_by_id(array($izin_id));
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if (!empty($flight_no_from)) {
                if ($flight_no_from <> $this->input->post('flight_no')) {
                    ...
                    // redirect
                    redirect('izin_pending_internasional/migrasi/rute_data/' . $this->input->post('registrasi_id') . '/' . $this->input->post('izin_id'));
                }
            }
    ...
    public function review($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_pending_internasional/migrasi/review.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id, $this->flow_id);
        $result = $this->m_migrasi->get_registrasi_pending_by_id($params);
        if (empty($result)) {
    ...
    public function send_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_migrasi->update_izin_process($params, $where)) {
                // insert next flow
        ...
        // default redirect
        redirect("izin_pending_internasional/migrasi/review/" . $registrasi_id);
    }
