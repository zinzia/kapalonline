...
class fa_aunbln_kasubdit extends ApplicationBase {
    ...
    public function index($data_id = "") {
        ...
        $this->smarty->assign("template_content", "task/fa_aunbln_kasubdit/detail.html");
        // get detail task
    ...
    public function edit_process() {
        ...
        // default redirect
        redirect("task/fa_aunbln_kasubdit/index/" . $data_id);
    }
    ...
    public function back_process($data_id = "", $process_id = "") {
        ...
        if ($this->m_task->action_update($params)) {
            ...
            $next_role = $this->m_task->get_role_next_from(array($next_flow));
            // send mail to next flow
            $this->_send_mail_to_regulator($data_id, $next_role['role_id']);
            // success
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // default redirect
    ...
    public function finish_process($data_id = "", $process_id = "") {
        ...
        if ($task['services_cd'] == "C" || $task['services_cd'] == "E") {
            // success
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("error", "Maaf data ini hanya dapat di validasi Direktur");
            // default redirect
            redirect("task/manager/");
        }
        ...
        $trim = "4";
        // get fa group
        $group = $this->m_task->get_fa_group('2');
        // update
        $params = array('approve', 'done', $this->com_user['user_id'], $process_id);
        if ($this->m_task->action_update($params)) {
            // total registration
