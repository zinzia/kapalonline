...
class frekuensi_delete extends ApplicationBase {
    ...
    public function index($registrasi_id = "") {
        ...
        $this->smarty->assign("template_content", "izin_internasional/frekuensi_delete/index.html");
        // load javascript
    ...
    public function edit_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_registrasi->update_izin_registrasi($params, $where)) {
                ...
                // default redirect
                redirect("izin_internasional/frekuensi_delete/list_rute/" . $this->input->post('registrasi_id'));
            } else {
                // default error
        ...
        // default redirect
        redirect("izin_internasional/frekuensi_delete/index/" . $this->input->post('registrasi_id'));
    }
    ...
    public function list_rute($registrasi_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "izin_internasional/frekuensi_delete/list.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_registrasi->get_registrasi_waiting_by_id($params);
        if (empty($result)) {
    ...
    public function rute_add_process($registrasi_id = "", $kode_frekuensi = "") {
        ...
        if (!empty($izin_rute)) {
            ...
            if ($this->m_registrasi->insert_izin_rute($params)) {
                ...
                redirect('izin_internasional/frekuensi_delete/list_rute/' . $registrasi_id);
            }
        }
    ...
    public function list_rute_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // redirect
            redirect("izin_internasional/frekuensi_delete/list_files/" . $this->input->post('registrasi_id'));
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function list_files($registrasi_id = "") {
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "izin_internasional/frekuensi_delete/files.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_registrasi->get_registrasi_waiting_by_id($params);
        if (empty($result)) {
    ...
    public function files_process() {
        ...
        // default redirect
        redirect("izin_internasional/frekuensi_delete/list_files/" . $registrasi_id);
    }
    ...
    public function files_download($data_id = "", $ref_id = "") {
        ...
        if (empty($result)) {
            redirect("izin_internasional/frekuensi_delete/list_files/" . $data_id);
        }
        // filepath
    ...
    public function files_next() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
            // default redirect
            redirect("izin_internasional/frekuensi_delete/review/" . $registrasi_id);
        } else {
            // default error
            $this->tnotification->sent_notification("error", "Data gagal disimpan");
        }
    ...
    public function review($registrasi_id = "") {
        ...
        $this->_set_page_rule('R');
        // set template content
        $this->smarty->assign("template_content", "izin_internasional/frekuensi_delete/review.html");
        // get detail data
        $params = array($registrasi_id, $this->com_user['airlines_id'], $this->group_id);
        $result = $this->m_registrasi->get_registrasi_waiting_by_id($params);
        if (empty($result)) {
