...
class operatorlogin extends ApplicationBase {
            ...
            if ($this->input->post($token_nm) == $csrf_token) {
                if ($captcha_data['word'] == $captcha AND $captcha_data['ip_address'] == $_SERVER["REMOTE_ADDR"] AND $captcha_data['captcha_time'] > $expiration) {
                    ...
                    if (!empty($result)) {
                        ...
                        // set session
                        $this->tsession->set_userdata('session_fa_online', array('user_id' => $result['user_id'], 'role_id' => $result['role_id']));
                        // insert login time
    ...
    public function logout_process() {
        // user id
        $user_id = $this->tsession->userdata('session_fa_online');
        // insert logout time
        $this->m_account->update_user_logout($user_id);
        // unset session
        $this->tsession->unset_userdata('session_fa_online');
        // output
