...
class m_operator extends CI_Model {
    ...
    function update_data_pribadi($params) {
        $sql = "UPDATE com_user SET operator_name = ?, operator_gender = ?, 
                ...
                user_mail = ?, member_status = ?, jabatan = ?, pelabuhan_id = ?
                WHERE user_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function get_total_member($params) {
        $sql = "SELECT COUNT(*)'total' 
                ...
                (
                    ...
                    AND user_st = 'member' AND reg_st = 1
                    GROUP BY a.user_id
                ) result";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
    ...
    function get_list_operator($params) {
        $sql = "SELECT * 
                FROM 
                (
                    SELECT a.*, b.pelabuhan_nm, b.pelabuhan_kd
                    FROM com_user a
                    LEFT JOIN pelabuhan b ON a.pelabuhan_id = b.pelabuhan_id
                    WHERE operator_name LIKE ? AND a.pelabuhan_id LIKE ? AND user_st = 'operator' AND operator_name != '' 
                    GROUP BY a.user_id
                ) result
                ORDER BY operator_name ASC
                LIMIT ?, ?";
    ...
    function get_all_direktorat_angkutan_udara() {
        $sql = "SELECT * FROM com_preferences WHERE pref_group = 'direktorat' AND pref_nm = 'pelabuhan_id'";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
