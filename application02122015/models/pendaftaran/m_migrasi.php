...
class m_migrasi extends CI_Model {
    ...
    function get_list_izin_rute_waiting_by_id($params) {
        $sql = "SELECT a.izin_id, a.airlines_id, a.kode_izin, a.kode_frekuensi, a.registrasi_id, 
                ...
                a.izin_st, a.izin_start_date, a.izin_expired_date, a.izin_rute_start, a.izin_rute_end, 
                a.pairing,
    ...
    function get_list_izin_data_waiting_by_id($params) {
        $sql = "SELECT rute_id, izin_id, rute_all, tipe, capacity, flight_no, etd, eta, doop, roon, 
                ...
                FROM izin_data a
                WHERE a.izin_id = ?
                ORDER BY rute_all ASC, start_date ASC, etd ASC";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
